...
class ShowBar12 extends React.Component{
    async getData(){
        try {
    ...
    renderBar(barDataTotal,format,max,length){
        //console.log("数据："+max);
        max = 17;
        let Bar = echarts.init(document.getElementById('bar12'));
        let yData_deploy = []
        let barData  = []
        let barDatatwo = []
    ...
    render(){
        ...
        return(
            <div className={css(styles.container)}>
                <div className={css(styles.header)}>{/*{this.props.barName}*/}实验室当天任务数/已完成数</div>
                {/*{spanArr}*/}
                <div className={css(styles.showDiv)}>
                    <div className={css(styles.tubiao)} id='bar12' style={{width:'355px',height:'250px'}}></div>
                </div>
            </div>
        )
    }
