...
class ShowBar3 extends React.Component{
    async getData(){
        try {
    ...
    renderBar(barDataTotal,format,max,length){
        //console.log("数据："+max);
        max = 0.016;
        let Bar = echarts.init(document.getElementById('bar3'));
        let yData_deploy = []
        let barData  = []
        let barDatatwo = []
        ...
        barData = [{name:"午阳食品",value:0.006,itemStyle:{
            normal:{color:'#40A9ED'}
        }},{name:"长图食品",value:0.011,itemStyle:{
            normal:{color:'#40A9ED'}
        }},{name:"常胜食品）",value:0.013,itemStyle:{
            normal:{color:'#40A9ED'}
        }},{name:"东升食品",value:0.014,itemStyle:{
            normal:{color:'#40A9ED'}
        }},{name:"一零六食品）",value:0.016,itemStyle:{
            normal:{color:'#40A9ED'}
        }}]
        barDatatwo = [{name:"机场货管",value:80},{name:"京监管处",value:75},{name:"京关展览",value:70},{name:"京关保税",value:65},{name:"京涉外处",value:59}]
        yData_deploy = ["午阳食品","长图食品","常胜食品","东升食品","一零六食品"]
        ...
        //barConfig3.yAxis.position = "left"   //modify by liulinqing 修改lable显示位置
    ...
    render(){
        ...
        return(
            <div className={css(styles.container)}>
                <div className={css(styles.header)}>{/*{this.props.barName}*/}企业不合格率排行</div>
                {/*{spanArr}*/}
                <div className={css(styles.showDiv)}>
                    <div className={css(styles.tubiao)} id='bar3' style={{width:'355px',height:'230px'}}></div>
                </div>
            </div>
        )
    }
