...
class ShowBar6 extends React.Component{
    async getData(){
        try {
    ...
    renderBar(barDataTotal,format,max,length){
        console.log("数据："+max);
        max = 1;
        let Bar = echarts.init(document.getElementById('bar6'));
        let yData_deploy = []
        let barData  = []
        let barDatatwo = []
        ...
        return(
            <div className={css(styles.container)}>
                ...
                <div className={css(styles.showDiv)}>
                    <div className={css(styles.tubiao)} id='bar6' style={{width:'418px',height:'190px'}}></div>
                </div>
            </div>
        )
    }
